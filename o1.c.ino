...
#define TOPICO_SUBSCRIBE "botao1"     
#define TOPICO_PUBLISH   "acc"
...
//const char* SSID = "Rede_B";
//const char* PASSWORD = "rastafari";
...
void mqtt_callback(char* topic, byte* payload, unsigned int length) 
{
    ...
    Serial.print(topic);
    Serial.println();
    Serial.print("Content from action is: ");
    Serial.print(msg);
    Serial.println();
    Serial.println();
    if( msg.equals("aceitar"))
    {
      permition = true;
    }
...
void loop() {
  ...
  if(red_state==HIGH)
  {
    ...
    //JsonVariant variant = data;
    //const char* s = variant.as<char*>();
    String s = String(GyX);
    s+='\n';
    s+=String(GyY);
    s+='\n';
    ...
    MQTT.publish("acc",char_array);
  }

  if(green_state==HIGH)
  {
    float d = ultrasonic.distanceRead();
    Serial.print("Distancia: "); // Escreve texto na tela
    Serial.print(d);// distância medida em cm
    Serial.println("cm"); // escreve texto na tela e pula uma linha
    delay(1000); // aguarda 1s
    char charVal[10];
    dtostrf(d, 4, 3, charVal);
    MQTT.publish("approx",charVal);
  }
  reconnectMQTT();
  reconnectWiFi();

  switchState = digitalRead(button);
  //digitalWrite(red,switchState);
   /*
   if (switchState == 1)
   {
